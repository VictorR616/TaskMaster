...
class TestViews(TestCase):
    ...
    def test_update_priorities(self):
        response = self.client.post(self.update_url, {"name": "Baja"})

        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/todo_task/priorities/"
    ...
    def test_create_priorities(self):
        ...
        response = self.client.post(
            self.create_url, {"name": "Prioridad absoluta no encontrada"}
        )
    ...
    def test_delete_priority_no_id(self):
        ...
        response = self.client.post(
            self.delete_url
        )  # Nota que aquí no proporcionamos un ID válido
